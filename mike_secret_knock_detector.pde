...
const int switchMotor = 8;       // If this is high we program a new code.
...
int now=0;
int recordTime=0;
int lockTurnTime = 1650;      // milliseconds that we run the motor to get it to go a half turn.
...
void loop() {
  ...
  if (digitalRead(switchMotor)==HIGH){
      ...
      while (digitalRead(programSwitch)==HIGH){
          digitalWrite(motor1, HIGH);                  //FORWARD
          digitalWrite(motor2, LOW);                   //FORWARD
      }  
      if (recordTime == 1){
         now=millis();
         lockTurnTime = now - startTime;
         Serial.print("result = ");
         Serial.println(lockTurnTime);
         recordTime=0;
         digitalWrite(motor1, LOW);
         digitalWrite(motor2, LOW);
         delay(timeDelay);
      }
          digitalWrite(motor1, LOW);                //REVERSE
          digitalWrite(motor2, HIGH);               //REVERSE
  } 
  else{
    digitalWrite(motor1, LOW);
    digitalWrite(motor2, LOW);
...
void triggerDoorUnlock(){
  ...
  digitalWrite(motor1, LOW);
  digitalWrite(motor2, LOW); 
  delay (timeDelay); //shorter tim        // Wait a bit.
  digitalWrite(motor1, HIGH);             //FORWARD
  digitalWrite(motor2, LOW);              
  delay (lockTurnTime);                    // Wait a bit.
